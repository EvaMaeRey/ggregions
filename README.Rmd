...
Maps are some of the most intuitive data visualizations and some of the first visualizations people will come across in their lives.  There is great support for mapping in R and data viz library {ggplot2}.  

However, producing a simple map like a choropleth can feel much more effortful than producing other plots, like a scatterplot for example.  A scatter plot is produced by 
...
Packages that might take avantage of `write_geom_region_locale()` and similar offerings include
...
Since the geo packages listed above have many users, the impacts of `{ggregions}` could be significant.
